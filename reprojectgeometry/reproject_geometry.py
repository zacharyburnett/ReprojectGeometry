...
def reproject_geometry(
    ...
    if geometry_type == 'Point':
        coordinates = reproject_points(
            numpy.expand_dims(coordinates, axis=0),
            input_crs,
            output_crs,
        )[0]
        ...
        for part_index, part_length in enumerate(part_lengths):
            coordinates[part_index] = \
                reprojected_coordinates[coordinate_index:
                                        coordinate_index + part_length]
            coordinate_index += part_length
    elif geometry_type == 'MultiPolygon':
        coordinates = [list(polygon) for polygon in coordinates]
        part_lengths = [[len(shell) for shell in polygon]
                        for polygon in coordinates]
        reprojected_coordinates = reproject_points(
            numpy.concatenate(
                [shell for polygon in coordinates for shell in polygon],
                axis=0,
            ),
            input_crs,
            output_crs,
        ...
        for part_index, shell_lengths in enumerate(part_lengths):
            for shell_index in range(len(shell_lengths)):
                ...
                coordinates[part_index][shell_index] = \
                    reprojected_coordinates[coordinate_index:
    ...
    if 'Polygon' in geometry_type:
        coordinates = [
            part for part in coordinates
            if sum(len(shell) for shell in part) > 0
        ]
